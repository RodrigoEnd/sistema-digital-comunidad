...
class SistemaFaenas:
    ...
    def cargar_datos(self):
        try:
            if seguridad.archivo_existe(ARCHIVO_FAENAS):
    ...
    def guardar_datos(self, mostrar_alerta=False):
        try:
            ...
            registrar_error('faenas', 'guardar_datos', str(e))
            if mostrar_alerta:
    ...
    def _crear_panel_resumen(self, parent):
        ...
        años_disponibles = sorted({datetime.now().year} | {self._anio_de_faena(f) for f in self.faenas})
    ...
    def actualizar_resumen_anual(self):
        ...
        for faena in self.faenas:
            ...
            for p in faena.get('participantes', []):
                ...
                puntos.setdefault(clave, {'folio': p.get('folio', ''), 'nombre': p.get('nombre', ''), 'puntos': 0})
                
                # Aplicar el peso_aplicado si existe (0.9 para quien contrató a habitante, 1.0 para todos los demás)
    ...
    def _puede_editar_faena(self):
        ...
        try:
            fecha_faena_str = self.faena_seleccionada.get('fecha', '')
            if not fecha_faena_str:
                return True  # Sin fecha, permitir edición
            
            fecha_faena = datetime.strptime(fecha_faena_str, "%Y-%m-%d").date()
            hoy = datetime.now().date()
            dias_transcurridos = (hoy - fecha_faena).days
            
            # Si han pasado más de 7 días, no permitir edición
    ...
    @staticmethod
    def _anio_de_faena(faena):
        fecha_txt = faena.get('fecha', '')
        try:
            return datetime.strptime(fecha_txt, "%Y-%m-%d").year
        except Exception:
            return datetime.now().year

    @staticmethod
    def _color_por_puntaje(valor, max_valor):
        # Gradiente simple rojo -> verde
        ratio = max(0.0, min(1.0, valor / max_valor))
        r1, g1, b1 = (220, 53, 69)   # rojo
        r2, g2, b2 = (40, 167, 69)   # verde
